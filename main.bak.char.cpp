...
int main() {
  ...
  OuvrireSess OuvrirSession = (OuvrireSess) GetProcAddress( hGetProcIDDLL, "OuvrirSession");
  ...
  typedef int (*LireDonnee)(HANDLE, char*, char*, int, int);
  ...
  typedef int (*LireDonneeP)(HANDLE, char*, char*, int, int);
  ...
  int retOuvrireSession = OuvrirSession( sess, sessionType /*1 ou 2 ou 3*/);  
  ...
  char valeurAS[16] = "000000000000000";
  LireDonneeAS(sess, "CS.ASSURE.NOMLA", valeurAS, strlen(valeurAS), sessionType /*1 ou 2 ou 3*/);
  std::cout << "LireDonneeAS a etait appellee!\n";
  system("pause");
  std::cout << "la valeur LireDonneeAS est: " << *valeurAS << std::endl;
  system("pause");

  // La fonction LireDoneePS
  // les parametres sont
  // (session, label, valeur, longueur, typeDeSession)
  char valeurPS[16] = "000000000000000";
  LireDonneePS(sess, "CS.ASSURE.NOMLA", valeurPS, strlen(valeurPS), sessionType /*1 ou 2 ou 3*/);
  std::cout << "LireDonneePS a etait appellee!\n";
  system("pause");
  std::cout << "la valeur LireDonneePS est: " << *valeurPS << std::endl;
  system("pause");
